            ...
            <q-step :done="step > 1" :error="stepErrors.step1" :name="1" icon="settings" title="">
              ...
              <Form ref="step1" :initial-values="formData">
                <Field #default="{ errorMessage, value, field }" label="nom de l'espace" name="name" rules="required">
                  <q-input v-model="formData.name" :error="!!errorMessage" :error-message="errorMessage" dense filled label="Nom de l'espace" v-bind="field"></q-input>
                </Field>
              </Form>
            </q-step>
...
const formData = ref<FormCreateWorkspace>({ name: null });
